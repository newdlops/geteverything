...
프롬프트 `product-sft-5`부터 검증 상품명·모델번호가 프롬프트 예제에 있는지도 검사한다.
검증의 단일 상품이 학습의 혼합·선택형 제목에 등장할 때에도 해당 가족 전체를 검증으로 옮긴다.
검증에만 있는 상품을 학습에서 비상품으로 외우는 문제를 막는다.
생성 평가는 기존 운영 조건, 후보와 같은 프롬프트의 원본 모델, 후보 어댑터의 세 조건을 비교한다.
후보는 두 대조군 모두보다 개선되고 기존 정답을 잃지 않아야 한다.
llama.cpp의 요청 `lora: []`는 기본 어댑터를 상속하므로 원본 모델 비교에 사용하지 않는다.
명시적인 `[{"id":0,"scale":0.0}]`으로 모든 어댑터를 끄며, 요청 코드도 재개 검증 해시에 포함한다.
2026-09-22 이전의 빈 배열을 사용한 대조 결과는 학습 전후 비교 근거로 사용할 수 없다.

`product-sft-6`은 상품/비상품에 같은 쇼핑몰 표기 변형을 사용하고, 학습·후보 추론 모두에서
알려진 쇼핑몰 태그와 가격/배송 정보만 담은 괄호를 제거한다. 용량이나 모델번호가 섞인 괄호는 보존한다.
어댑터가 없는 운영 대조군은 기존 입력을 유지하며, 동일 프롬프트 원본 대조군에는 후보와 같은 전처리를 적용한다.
같은 평가 조건에서 입력이 완전히 동일한 요청은 생성 결과를 재사용한다. 원래 제목별 정규화와 채점은 각각 수행하고,
지연 시간에는 최초 실제 추론 시간을 기록한다. `inference_requests`와 행별 `inference_reused`로 실제 호출 수를 구분한다.
