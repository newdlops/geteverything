#!/usr/bin/env python3
"""CPU/MPS LoRA on Qwen's final block, with reusable CPU prefix activations."""
import argparse
import hashlib
import json
import math
...
def main():
    ...
    parser.add_argument('--training-device', choices=('cpu', 'mps'), default='cpu')
    ...
    if args.training_device == 'mps' and not torch.backends.mps.is_available():
        raise RuntimeError('MPS is unavailable; use --training-device cpu')
    ...
    progress = {'run_id':run_id, 'dataset_sha256':data['sha256'], 'base_revision':checkpoint['revision'],
                ...
                'training_device':args.training_device,'cpu_threads':args.threads}
    def report(**values):
    ...
    import gc
    gc.collect()
    for module in (layer, norm, head):
        module.to(args.training_device)
    trained = [(name, value) for name,value in model.named_parameters() if value.requires_grad]
    ...
    def loss_for(index):
        ...
        entry = move_tensors(entry, args.training_device)
